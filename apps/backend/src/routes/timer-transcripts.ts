...
router.post('/generate-questions', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Generate creative questions using selected AI provider
    ...
    res.status(200).json({
      ...
      data: {
        ...
        providerLabel: actualProvider === 'ollama' ? '🦙 Generated by Ollama (Local AI)' : '🤖 Generated by Gemini AI'
      },
      message: `Generated ${savedQuestions.length} creative questions from timer transcript using ${actualProvider.toUpperCase()}`
    });
